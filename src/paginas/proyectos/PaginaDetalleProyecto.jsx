...
import { escucharTramitaciones, crearTramitacion, eliminarTramitacion } from '../../servicios/tramitaciones'
import { TIPOS_TRAMITACION, LISTA_TIPOS } from '../../constantes/tramitaciones'
...
export default function PaginaDetalleProyecto() {
  ...
  useEffect(() => {
    if (!id) return
    const cancelar = escucharTramitaciones(id, setTramitaciones)
    return cancelar
  }, [id])
  ...
  async function manejarCrearTramitacion() {
    ...
    try {
      await crearTramitacion(id, tipoSeleccionado)
  ...
  async function manejarEliminar(tramId, tramNombre, e) {
    ...
    try { await eliminarTramitacion(id, tramId); toast.success('Eliminada') }

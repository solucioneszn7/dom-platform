...
export default function PaginaCertificaciones() {
  ...
  const puedeEditarCert = permisos.editarCert
  const puedeEliminarCert = permisos.eliminarCert
  ...
  async function crearCert() {
    if (!puedeEditarCert) { toast.error('Sin permiso para crear certificaciones'); return }
    setCreando(true)
    try { const n = await generarCertificacion(proyectoId); setCerts(prev => [n, ...prev]); toast.success(`Cert. #${n.numero} creada`) }
  ...
  async function cambiarEst(certId, estado) {
    ...
    if ((estado === 'aprobada' || estado === 'facturada') && !puedeAprobar) { toast.error('Sin permiso para aprobar'); return }
    try {
  ...
  async function borrarCert(certId) {
    if (!puedeEliminarCert) { toast.error('Sin permiso para eliminar'); return }
    const cert = certs.find(c => c.id === certId)
    // Admin/Director can delete any state, others only borrador
    if (cert.estado !== 'borrador' && !['admin', 'director_general'].includes(rol)) {
      toast.error('Solo puedes eliminar borradores'); return
    }
    if (!window.confirm(`¿Eliminar Cert. #${cert.numero}? ${cert.estado !== 'borrador' ? '⚠️ ESTA CERTIFICACIÓN YA FUE PROCESADA' : ''}`)) return
    try {
  ...
  function empezarEditar(cert) {
    if (!puedeEditarCert) { toast.error('Sin permiso'); return }
    // Admin/Director can edit any, others only borrador/pendiente
    if (!['borrador', 'pendiente'].includes(cert.estado) && !['admin', 'director_general'].includes(rol)) {
      toast.error('Solo puedes editar borradores o pendientes'); return
    }
    setEditando(cert.id)
  ...
  async function guardarEdicion() {
    try {
      ...
      setEditando(null); toast.success('Actualizada')
    } catch { toast.error('Error') }
  }
  ...
  if (!proyectoId) {
    return (
      <div className="space-y-6">
        <div><h1 className="text-2xl font-bold text-gray-900 tracking-tight">Certificaciones</h1>
          <p className="text-sm text-gray-400 mt-1">Rol: {rol} · {puedeEditarCert ? '✅ Editar' : '🔒 Solo lectura'} · {puedeEliminarCert ? '✅ Eliminar' : '🔒 Sin eliminar'}</p></div>
        {cargando ? <Cargando /> : (
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold text-gray-900">Certificaciones</h1><p className="text-[12px] text-gray-400">{proyectoNombre} · {puedeEditarCert ? 'Puede editar' : 'Solo lectura'}</p></div>
        </div>
        {puedeEditarCert && <Boton icono={Plus} tamano="sm" cargando={creando} onClick={crearCert}>Generar Certificación</Boton>}
      </div>

      {/* Summary */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[['Presupuesto', presTotal.toLocaleString('es-ES', { maximumFractionDigits: 0 }), DollarSign],
          ['Certificado', totalCert.toLocaleString('es-ES', { maximumFractionDigits: 0 }), FileCheck2],
          ['Pendiente', (presTotal - totalCert).toLocaleString('es-ES', { maximumFractionDigits: 0 }), Clock],
          ['% Cert.', `${pctCert}%`, CheckCircle],
        ].map(([l, v, I]) => (
          <div key={l} className="rounded-lg px-3 py-2.5 bg-gray-50 border border-gray-200">
            <div className="flex items-center gap-1.5 mb-0.5"><I className="h-3.5 w-3.5 text-gray-500" /><span className="text-[10px] font-medium uppercase text-gray-500">{l}</span></div>
      ...
      {/* List with RBAC edit/delete */}
      <Tarjeta>
        <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-gray-800">Historial</h2>
          <div className="flex items-center gap-2 text-[10px] text-gray-400">
            {puedeEditarCert && <span className="flex items-center gap-1"><Edit2 className="h-3 w-3" />Editar</span>}
            {puedeEliminarCert && <span className="flex items-center gap-1"><Trash2 className="h-3 w-3" />Eliminar</span>}
            {puedeAprobar && <span className="flex items-center gap-1"><Shield className="h-3 w-3" />Aprobar</span>}
          </div>
        </div>
        {certs.length === 0 ? <div className="py-12 text-center"><FileText className="h-10 w-10 text-gray-200 mx-auto mb-3" /><p className="text-sm text-gray-400">Sin certificaciones.</p></div> : (
          <div className="divide-y divide-gray-50">{certs.map(cert => {
            ...
            // Can this user edit/delete THIS cert?
            const canEdit = puedeEditarCert && (['borrador', 'pendiente'].includes(cert.estado) || ['admin', 'director_general'].includes(rol))
            const canDelete = puedeEliminarCert && (cert.estado === 'borrador' || ['admin', 'director_general'].includes(rol))

            return (
              <div key={cert.id} className="px-5 py-3.5 hover:bg-gray-50/50 transition-colors">
                {isEditing ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between"><span className="text-[13px] font-semibold text-gray-900">Editando Cert. #{cert.numero}</span>
                      <div className="flex items-center gap-2"><Boton tamano="xs" icono={Save} onClick={guardarEdicion}>Guardar</Boton><button onClick={() => setEditando(null)} className="p-1 rounded-md hover:bg-gray-100"><X className="h-4 w-4 text-gray-400" /></button></div></div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div><label className="text-[10px] text-gray-400 uppercase block mb-1">Mes</label><input value={formEdit.mes || ''} onChange={ev => setFormEdit({ ...formEdit, mes: ev.target.value })} className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm focus:border-dom-500 focus:outline-none" /></div>
                      <div><label className="text-[10px] text-gray-400 uppercase block mb-1">Año</label><input type="number" value={formEdit.anio || ''} onChange={ev => setFormEdit({ ...formEdit, anio: +ev.target.value })} className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm focus:border-dom-500 focus:outline-none" /></div>
                      <div><label className="text-[10px] text-gray-400 uppercase block mb-1">Origen</label><input type="number" value={formEdit.importeOrigen || ''} onChange={ev => setFormEdit({ ...formEdit, importeOrigen: +ev.target.value })} className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm focus:border-dom-500 focus:outline-none" /></div>
                      <div><label className="text-[10px] text-gray-400 uppercase block mb-1">Actual</label><input type="number" value={formEdit.importeActual || ''} onChange={ev => setFormEdit({ ...formEdit, importeActual: +ev.target.value })} className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm focus:border-dom-500 focus:outline-none" /></div>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 cursor-pointer flex-1" onClick={() => setCertSel(cert)}>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {cert.estado === 'aprobada' && <Lock className="h-3 w-3 text-amber-500" />}
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1">
                        {canEdit && <button onClick={() => empezarEditar(cert)} className="p-1.5 rounded-md text-gray-400 hover:text-dom-600 hover:bg-dom-50" title="Editar"><Edit2 className="h-3.5 w-3.5" /></button>}
                        {canDelete && <button onClick={() => borrarCert(cert.id)} className="p-1.5 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50" title="Eliminar"><Trash2 className="h-3.5 w-3.5" /></button>}
                      </div>
                    </div>
                  </div>
                )}
      ...
      {certSel && (
        <Tarjeta>
          <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {certSel.estado === 'borrador' && puedeEditarCert && <Boton variante="secundario" tamano="xs" icono={Send} onClick={() => cambiarEst(certSel.id, 'pendiente')}>Enviar</Boton>}
              {certSel.estado === 'pendiente' && puedeAprobar && <Boton variante="exito" tamano="xs" icono={CheckCircle} onClick={() => cambiarEst(certSel.id, 'aprobada')}>Aprobar</Boton>}
              {certSel.estado === 'aprobada' && puedeAprobar && <Boton tamano="xs" icono={FileCheck2} onClick={() => cambiarEst(certSel.id, 'facturada')}>Facturar</Boton>}
          ...
          <TarjetaCuerpo>
            ...
            {certSel.lineas?.length > 0 && (
              <div className="max-h-[250px] overflow-y-auto"><table className="w-full text-[11px]"><thead><tr className="border-b border-gray-100 text-gray-400 uppercase text-[10px]"><th className="text-left py-1.5 px-1">Código</th><th className="text-left py-1.5 px-1">Partida</th><th className="text-right py-1.5 px-1">Avance</th><th className="text-right py-1.5 px-1">Importe</th><th className="text-left py-1.5 px-1">Empresa</th></tr></thead>
                <tbody>{certSel.lineas.map((l, i) => <tr key={i} className="border-b border-gray-50"><td className="py-1 px-1 font-mono text-dom-600">{l.codigo}</td><td className="py-1 px-1 text-gray-700 max-w-[120px] truncate">{l.nombre}</td><td className="py-1 px-1 text-right text-gray-600">{l.avanceAcumulado}/{l.cantidadPresupuestada}</td><td className="py-1 px-1 text-right font-semibold text-gray-800">{(l.importeEjecutado || 0).toFixed(0)}</td><td className="py-1 px-1 text-gray-400">{l.empresa || '—'}</td></tr>)}</tbody></table></div>
            )}
            {/* Workflow */}

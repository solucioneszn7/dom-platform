...
export default function PaginaTablero() {
  ...
  return (
    <div className="h-[calc(100vh-3rem)] -mt-6 -mx-4 sm:-mx-6 lg:-mx-10 overflow-hidden flex flex-col">
      ...
      {/* Banner de contexto (licitación o proyecto vinculado) — glass */}
      {contexto && (
        <div
          className="flex-shrink-0 px-5 py-2.5 flex items-center gap-3 border-b border-white/[0.05]"
          style={{
        ...
        >
          <div className={`h-2 w-2 rounded-full shadow-md ${contexto.tipo === 'proyecto' ? 'bg-emerald-400 shadow-emerald-400/50' : 'bg-violet-400 shadow-violet-400/50'}`} />
          <span className="text-[10px] text-white/45 uppercase font-bold tracking-[0.12em]">
          ...
          {contexto.licitacionIVA > 0 && (
            <span className="text-[11px] font-mono text-violet-200 bg-violet-500/15 px-2 py-0.5 rounded-md ring-1 ring-violet-500/25 ml-auto">
          ...
          {nuevasTareas > 0 && (
            <span
              className="text-[10px] font-bold text-white px-2.5 py-1 rounded-full flex items-center gap-1 shadow-lg"
